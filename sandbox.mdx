...
Every sandbox request uses the same published key — there's nothing to create and nothing to keep secret:

```text
kn_sandbox_REPLACE_WITH_PUBLISHED_KEY
```
...
curl https://api.knock2.ai/v1/sandbox/accounts \
  -H "Authorization: Bearer kn_sandbox_REPLACE_WITH_PUBLISHED_KEY"
```
...
response = requests.get(
    ...
    headers={"Authorization": "Bearer kn_sandbox_REPLACE_WITH_PUBLISHED_KEY"},
)
print(response.json())
```
...
Call `GET /v1/sandbox/tenants` to look up that child tenant's `product_slug` rather than hardcoding it.
...
`event_type` accepts the same values as a real webhook subscription (`account.identified`, `contact.identified`, `score.changed`, `play.triggered`, `signal.detected`, `tenant.limit_reached`). The response tells you whether delivery succeeded:
...
The payload is signed exactly like a real delivery — verify it the same way described in [Receive Real-Time Visitor Event Alerts via Webhooks](/guides/webhooks). Nothing is stored: there's no subscription to list or delete afterward, just the one test delivery.
