...
<Note>
  Most endpoints below have a sandbox counterpart under `/v1/sandbox/` that accepts the same parameters and returns the same shape against fixed dummy data — no API key provisioning required. See the [Sandbox](/sandbox) page.
</Note>
...
| `402` | `credit_limit_exceeded` | Your account has exhausted its credits for the billing period (`POST /v1/enrich`, `POST /v1/prospect`). |
| `402` | `account_inactive` | The workspace itself is deactivated (`is_product_slug_active = false`) — **every** `/v1` route 402s with this code, including `GET /v1/me`, until it's reactivated. |
