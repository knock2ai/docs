...
Reactivation sets `is_product_slug_active = TRUE`, restores the tenant's Visual Visitor plan, and re-enables the tracking script and visitor identification. The tenant's original API key was never revoked, so it resumes working immediately with no re-issuing step.
