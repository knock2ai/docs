...
This restores the tracking script, visitor identification, and the tenant's Visual Visitor plan. The original `api_key` — never revoked in the first place — resumes working immediately, no re-issuing needed. See the [Reactivate Tenant reference](/api-reference/tenants/activate-tenant) for the full response shape.
