...
<Tip>
  Don't have a Knock2 account yet, or just want to explore first? The [Sandbox](/sandbox) uses a shared public key (prefixed `kn_sandbox_` instead of `kn_live_`) that works against every core endpoint immediately, with realistic dummy data and no signup.
</Tip>

Log in to the [Knock2 dashboard](https://dashboard.knock2.ai/) and go to **Settings → API Keys**. Click **Create API Key**, enter a descriptive label (for example, `backend-production` or `zapier-integration`), and copy the key that appears.
